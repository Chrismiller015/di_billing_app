// [SOURCE: apps/web/src/pages/ReportsPage.tsx]
import React from "react";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import toast from "react-hot-toast";
import { FaSpinner, FaDownload, FaTrash } from "react-icons/fa";
import { fetchReports, downloadReport, deleteReport } from "../api";
...
export function ReportsPage() {
  ...
  const handleDelete = (reportId: string, reportName: string) => {
    if (window.confirm(`Are you sure you want to permanently delete the report "${reportName}"?`)) {
            ...
            {reportsQuery.data?.map(report => (
              <tr key={report.id} className="border-t border-slate-800">
                ...
                <td className="px-3 py-2">
                  ...
                  <button onClick={() => handleDelete(report.id, report.name)} className="p-2 text-rose-400 hover:bg-slate-700 rounded" title="Delete Report">
                    <FaTrash />
                  </button>
                </td>
              </tr>
